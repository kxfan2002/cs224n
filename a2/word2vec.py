...
def naiveSoftmaxLossAndGradient(
    ...
    Poc = softmax(np.dot(outsideVectors,centerWordVec)) #vx1
    loss= - np.log(Poc[outsideWordIdx])
    gradCenterVec = np.sum(outsideVectors*Poc.reshape(outsideVectors.shape[0],1),axis=0)-outsideVectors[outsideWordIdx] #1xn
    gradOutsideVecs = np.multiply(Poc.reshape(outsideVectors.shape[0],1) , centerWordVec.reshape(1,centerWordVec.shape[0]))
    # print((centerWordVec.reshape(1,centerWordVec.shape[0])).shape)
...
def negSamplingLossAndGradient(
    ...
    # print(negSampleU.shape)
    tmp=-np.dot(negSampleU,centerWordVec)
    tmp[0]*=-1
    uwsvc=sigmoid(tmp)
    uovc = sigmoid(np.dot(outsideVectors[outsideWordIdx].T,centerWordVec))
    loss = - np.sum(np.log(uwsvc))

    negSampleU[0]*=-1
    gradCenterVec = np.sum(negSampleU,axis = 0)- np.sum(np.tile(np.array(uwsvc),(negSampleU.shape[1],1)).T*negSampleU,axis=0)
    negSampleU[0]*=-1
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for i in outsideWords:
        los,gracv,graoc=word2vecLossAndGradient(centerWordVectors[word2Ind[currentCenterWord]],word2Ind[i],outsideVectors,dataset)
        loss += los
        gradCenterVecs[word2Ind[currentCenterWord]] += gracv
        gradOutsideVectors += graoc
    ...
    ### END YOUR CODE
    # print(loss,gradCenterVecs,gradOutsideVectors)
